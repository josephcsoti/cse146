...
class FP(_CondOpportunity):

    """False positive equalization"""
    short_name = "FP"

    def init(self, dataX, dataA, dataY):
        dataA_fp = dataA.copy(deep=True)
        dataA_fp[dataY == 0] = 'all'
        super().init(dataX, dataA_fp, dataY, dataY.apply(lambda y : 'all'))